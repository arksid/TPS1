...
public class ControlsTutorialManager : MonoBehaviour
{
    ...
    void EnsureSteps()
    {
        if (steps == null || steps.Count == 0)
            FillDefaultSteps();        // 기본 단계 채우기
    }

    // ControlsTutorialManager.cs 안
    void EndTutorial()
    {
        _running = false;
        ...
        // 디버그(꼭 확인)
        Debug.Log("[ControlsTutorial] 튜토리얼 완료 - 마커/아웃라인 즉시 활성 시도",
            this);
        ...
        // 아웃라인/마커 표시: 튜토리얼 끝나야만 가능
        WaypointDirector.EnableHints();  // ★ 이제부터 표시 허용
